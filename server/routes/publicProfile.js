...
export function publicProfileHandler(req, res) {
  ...
  // THIS SURFACE IS A PREVIEW, NOT THE PUBLIC ONE.
  //
  // Cloudflare Pages is where coach links point, and it has to be: it holds an
  // immutable snapshot that survives this process restarting, redeploying,
  // rebuilding its profile directory, or being down. Render's copy is
  // regenerated by publishing and disappears with the dyno. A link to this
  // host in a coach's inbox would break on the next deploy, so the response
  // says what it is — cheap to read in a proxy log or a bug report, and it
  // costs nothing to anyone who never looks.
  res.set('X-Thriv3-Surface', 'preview');
