 ...
 *
 * ---------------------------------------------------------------------------
 * THE WRITE PATH IS BUILT, AND SINCE L8B-3 IT IS OPEN.
 *
 * From L7ZK the governance rule is that every new human disposition carries an
 * authenticated operator identity. This module was written when the
 * application had none: `server/index.js` mounted `cors()` and
 * `express.json()` and nothing else, no request carried an operator, and
 * `operator_users` was created by no code on this branch. So the door was
 * built shut -- `operatorFromRequest` returned null, `recordDisposition`
 * refused without an identity, and the POST answered 503 naming the
 * prerequisite.
 *
 * The prerequisite is now met. Phase 13K arrived with main: `attachOperator`
 * runs before this router, `requireOperator` guards the `/api` boundary above
 * it, and `req.operator` carries the signed-in identity. Nothing here changed
 * to open it -- `operatorFromRequest` always read `req.operator.id`, which is
 * what "the change is one function" meant. The 503 remains, and now fires only
 * when there genuinely is no session.
 *
 * `rosterGaps.js` reached the same conclusion in L7L and still writes
 * `operatorId: null`; bringing it across is its own decision, not this one.
 *
 * `rosterSeasonTrust.operator.test.js` binds the real merged application to a
 * port and proves the join: server-owned identity reaches `recordDisposition`,
 * a caller-supplied reviewer is refused rather than ignored, and optimistic
 * concurrency still holds.
 *
