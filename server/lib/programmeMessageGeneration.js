...
/**
 * THE PURSUIT STATES THAT PRODUCE CONTENT — D4.8.
 *
 * `planned` is a pursuit nothing has landed for yet; `active` is one that has
 * landed at least one accepted message and is eligible for its follow-up. Both
 * generate. `stopped` does not, and `waiting`/`completed` are unreachable.
 */
const GENERATES_FROM = Object.freeze([ATTEMPT_STATE.PLANNED, ATTEMPT_STATE.ACTIVE]);
...
export const GENERATION_REFUSAL = Object.freeze({
  ...
  /**
   * THE PURSUIT IS NOT BEING PURSUED.
   *
   * WIDENED IN D4.8 TO `planned` OR `active`, and the name is kept because
   * every caller and test already matches on it. It used to mean `planned`
   * alone, which was safe only because `active` was unreachable: nothing in the
   * build set it, so every attempt stayed planned for ever and follow-ups
   * generated by accident rather than by rule. D4.8 makes `active` real — one
   * accepted message — so the gate has to admit it or the second message of
   * every sequence becomes ungenerable.
   *
   * `stopped` is still refused: it is a decision somebody took. `waiting` and
   * `completed` remain refused too — they are in the schema's CHECK so the
   * column never needs a rebuild, not because anything may reach them.
   */
  CONTACT_ATTEMPT_NOT_PLANNED: 'CONTACT_ATTEMPT_NOT_PLANNED',
  /**
   * AN EARLIER MESSAGE TO THIS COACH IS UNRESOLVED — D4.8.
   *
   * A send in UNKNOWN_PROVIDER_RESULT may already be in the coach's inbox. The
   * database has always refused a second open message on that relationship, but
   * it refused at the very last write, as a unique-index violation, after a body
   * had been composed and an operator had been shown "send this now". This says
   * it at the point somebody asks, in words.
   *
   * It is not a new flag. `blocksRelationship` is D4.4's authority and this
   * reads it.
   */
...
export function generateProgrammeMessage({ programmeCampaignId, coachId, at = utcNow() } = {}) {
  ...
  if (!GENERATES_FROM.includes(attempt.state)) {
    throw fail(
   ...
   *
   * Asked BEFORE any content is composed. A relationship holding a message
   * whose provider result is unknown may already have delivered it, and the
   * honest thing to tell an operator is that this conversation is waiting to be
   * reconciled — not to write them a second email and refuse it at the end.
   */
  const blocking = unresolvedSendFor(attempt.outreach_id);
  if (blocking) {
    throw fail(
      GENERATION_REFUSAL.RELATIONSHIP_HAS_UNRESOLVED_SEND,
      'An earlier message to this coach is unresolved — we do not know whether the provider '
      + 'accepted it, so it may already be in their inbox. Resolve that message before writing '
      + 'another one to them.',
    );
  }
