...
// ---------------------------------------------------------------------------
// PAGE 6 — what happens after the first season
// ---------------------------------------------------------------------------

export function freshmanDevelopmentPage(k, model) {
  const pairs = model.freshman.progression;
  const r = model.freshman.retention;

  page(k, 'Programme evidence', 'After the first season',
    'What happens to a first-year here in their second year?');
  scope(k, [
    `${plural(pairs.length, 'first-year', 'first-years')} with a following season on file`,
    r ? `${r.stayed} of ${r.of} still on the roster` : null,
  ]);
  ...
  charts.slope(k, {
    ...
    unavailable: pairs.length ? null
      : 'no first-year here has a following season on file to compare against',
  });

  // Grouped by the band the player was in, which is the question a recruit who
  // expects to wait is actually asking. Counts only: there is no defensible
  // tolerance for calling a change "broadly similar", so none is asserted.
  const bands = [
    ['impact', `Played ${STARTER_MINUTES}+ minutes`],
    ['rotation', 'In the rotation'],
    ['fringe', 'Fringe minutes'],
    ['none', 'Did not play'],
  ];
  const bandOf = (mins) => (mins >= STARTER_MINUTES ? 'impact'
    : mins >= 200 ? 'rotation' : mins > 0 ? 'fringe' : 'none');

  const rows = bands.map(([key, label]) => {
    const group = pairs.filter((p) => bandOf(p.year1) === key);
    const measured = group.filter((p) => p.year2State === 'measured');
    return {
      label,
      n: group.length,
      more: measured.filter((p) => p.year2 > p.year1).length,
      less: measured.filter((p) => p.year2 <= p.year1).length,
      unrecorded: group.filter((p) => p.year2State === 'unrecorded').length,
      gone: group.filter((p) => p.year2State === 'gone').length,
    };
  }).filter((x) => x.n > 0);
  ...
  if (r) {
    k.body(`${r.stayed} of ${r.of} first-years were still on the roster a year later.`, { bold: true });
    k.note('A name leaving a roster can mean a move to another programme, an injury, a player who '
      + 'stopped playing, or a spelling we could not match. It is the one departure that cannot '
      + 'mean graduation, which is why it is worth showing — but it is not four separate facts, '
      + 'and none of them is proved here.');
  }
}
