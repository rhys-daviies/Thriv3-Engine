...
import { allStatuses, programmeKey, activeForSeason, TARGET_SEASON } from '../lib/programmeStatus.js';
 ...
 * failure.
 *
 * `season` excludes the same way, and for the same reason with a clock attached.
 * Wisconsin-La Crosse does not field men's soccer, Anna Maria closed after the
 * 2025-26 year, and Wisconsin-Oshkosh starts in 2027 — all three are correct
 * absences for 2026, and only the last is still a target next year. Passing a
 * season is what lets that be true; omitting it keeps the old behaviour exactly,
 * so a caller that does not care about time is unaffected. See
 * `shared/roster/programmeStatus.js`.
 *
 * Nothing else is excluded. In particular DUPLICATE REGISTRY ROWS ARE
...
export function rosterTargetUniverse({
  divisions = TARGET_DIVISIONS, sports = TARGET_SPORTS, includeInactive = false, season = null,
} = {}) {
  ...
  if (season == null || includeInactive) return rows;
  const statuses = allStatuses();
  return rows.filter((r) => activeForSeason(statuses.get(programmeKey(r.school, r.sport)) ?? null, season));
}
...
function main() {
  ...
  const season = Number(arg('season')) || TARGET_SEASON;   // one-arg helper: no default slot
  const rows = rosterTargetUniverse({ season });
   ...
   *
   * This report used to print one, labelled "with roster", counted over every
   * season. A programme last fetched in 2025 appeared under it, so the number
   * read as current-season completeness and was not: 1,745 against 1,607 for
   * 2026. Both are useful and neither may wear the other's name.
   */
  ...
  const yr = String(season ?? TARGET_SEASON);
  console.log(`\nROSTER TARGET UNIVERSE — ${rows.length} programmes active for ${yr}\n`);
  console.log(`  division    sport     programmes   ${yr} roster    any roster     missing ${yr}`);
  const by = new Map();
  for (const r of rows) {
    const k = `${r.division}|${r.sport}`;
    if (!by.has(k)) by.set(k, { n: 0, current: 0, historical: 0 });
    const e = by.get(k);
    e.n += 1;
    if (cur.get(r.school, r.sport, yr).n) e.current += 1;
    if (any.get(r.school, r.sport).n) e.historical += 1;
  }
  for (const [k, e] of by) {
    const [d, s] = k.split('|');
    console.log(`  ${d.padEnd(11)}${s.replace('-soccer', '').padEnd(10)}${String(e.n).padStart(10)}`
      + `${String(e.current).padStart(14)}${String(e.historical).padStart(13)}${String(e.n - e.current).padStart(14)}`);
  }
  const missing = rows.filter((r) => !cur.get(r.school, r.sport, yr).n);
  const never = missing.filter((r) => !any.get(r.school, r.sport).n);
  console.log(`\n  ${missing.length} programmes have no ${yr} roster — the current-season completeness gap.`);
  console.log(`  ${missing.length - never.length} of those are known from an earlier season; `
    + `${never.length} have never been fetched.`);
