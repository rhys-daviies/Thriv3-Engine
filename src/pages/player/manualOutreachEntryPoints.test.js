...
const manualButtons = () => buttonsIn(document.body)
  .filter((b) => b.textContent.includes(CREATE_EMAIL_DRAFT));
...
describe('a ranked programme WITH a relationship', () => {
  for (const [label, rel] of [
    ...
    it(`offers relationship outreach when it is ${label}`, async () => {
      ...
      expect(buttonsIn(card).some((b) => b.textContent.includes(CREATE_EMAIL_DRAFT))).toBe(true);
    });
  }
  ...
  it('opens the dialog against that relationship’s own id', async () => {
    ...
    stubFetch({ programmes: [rel] });
    await render();
    await click(buttonsIn(cardFor('Alpha')).find((b) => b.textContent.includes(CREATE_EMAIL_DRAFT)));
  ...
  it('offers it on exactly the card that has the relationship', async () => {
    ...
    expect(buttonsIn(cardFor('Bravo')).some((b) => b.textContent.includes(CREATE_EMAIL_DRAFT))).toBe(false);
  });
  ...
  it('matches the relationship by college id, not by name alone', async () => {
    ...
    stubFetch({ programmes: [rel] });
    await render();
    await click(buttonsIn(cardFor('Bravo')).find((b) => b.textContent.includes(CREATE_EMAIL_DRAFT)));
...
describe('the removed-programmes panel', () => {
  ...
  it('offers relationship outreach beside Keep in Top 100', async () => {
    ...
    expect(buttonsIn(list).some((b) => b.textContent.includes(CREATE_EMAIL_DRAFT))).toBe(true);
  ...
  it('opens the dialog for the suppressed relationship', async () => {
    ...
    await click([...buttonsIn(container)].find((b) => b.textContent.includes('Removed from this athlete')));
    const list = container.querySelector('[data-testid="suppressed-list"]');
    await click(buttonsIn(list).find((b) => b.textContent.includes(CREATE_EMAIL_DRAFT)));
...
describe('Specific Schools is unchanged', () => {
  it('still offers relationship outreach on every row', async () => {
    ...
    // The one action a specific request exists for stays at the glance.
    expect(buttonsIn(row).some((b) => b.textContent.includes(CREATE_EMAIL_DRAFT))).toBe(true);
    /**
     * F8b — Remove moved into the expansion with the other secondary
     * decisions. It is the only destructive control on this surface and no
     * longer sits one gap from a contact-policy button on a row being scanned.
     */
    await click(buttonsIn(row).find((b) => b.textContent.trim().startsWith('Details')));
  ...
  it('still opens against the relationship id', async () => {
    ...
    await click(tab('Specific Schools'));
    const row = container.querySelector('[data-testid="specific-school-row"]');
    await click(buttonsIn(row).find((b) => b.textContent.includes(CREATE_EMAIL_DRAFT)));
    expect(contextCalls()[0].path).toBe(`/api/players/${ATHLETE}/programmes/rel-Alpha/outreach`);
  });
});
