...
| Mailbox access | **The athlete authorises their own mailbox through a one-time consent link the operator sends** (decided 2026-09-08) | Resolves the tension D1 found between "send from the client's own mailbox" and "no athlete accounts": consent is not a session. The link is purpose-specific and short-lived, the OAuth consent happens directly with Google or Microsoft, and it grants no Thriv3 sign-in, no operator access and no dashboard — nothing but a verified mailbox associated with that athlete. The operator remains the campaign controller throughout. The connected mailbox's identity comes from the provider, never from a caller-supplied address, so budget and sender reputation cannot be accounted against one identity while sending through another. Implementation is Phase D2 onwards; nothing about the link is designed here. |
| Phase D base | **D2 branches from `main` after `integration/hosted-campaign` lands** (decided 2026-09-08) | The campaign engine and the hosted operator runtime grew on separate branches and were reconciled on an integration branch, not on trunk. Mailbox credentials must not be introduced against a base that is missing either half — encrypted refresh tokens in an application without authentication is a worse position than no tokens at all. So D2 waits for one authoritative trunk rather than branching again from `engagement-tracking`, an old integration head, or any other long-lived campaign branch. |
| Mailbox credentials | **Encrypted at rest, AES-256-GCM, bound to the mailbox** (D2, 2026-09-09) | A refresh token is the ability to send mail as the athlete until they revoke it, and they will not think to — so it is the only encrypted thing in the database, in a table of its own that no mailbox read joins to. The mailbox id and key version are bound in as AAD, so a ciphertext moved to another row fails to authenticate rather than decrypting into a working credential for the wrong person. `THRIV3_MAILBOX_KEY` is base64 of 32 bytes, provisioned by hand because Render's generated secrets cannot guarantee that length, and production refuses to start without a valid one. Identity is the provider's account id, never a caller-supplied address; revoking destroys the credential and keeps the record. No OAuth, no Google, no Microsoft and no transport exist yet — D2 is the foundation those flows will use, and nothing can create a mailbox but the trusted service layer. `connected_mailboxes.email_address` becomes B5's `sending_identity` in a later phase; it is not wired yet. |
| Mailbox authorisation | **Athlete authorises Gmail through a 30-minute single-use consent link** (D3, 2026-09-09) | Consent is a capability, not a session: the link authorises one action for one athlete at one provider and grants no Thriv3 access. The token and the OAuth state are separate secrets, both stored only as HMACs under the session secret, so a database dump yields no usable link. PKCE S256 is used despite the confidential client, because the secret proves the exchange came from our server and the verifier proves it came from the browser that started it. Identity is the ID token subject with a verified email; nothing the browser sends can name the account. Scopes are openid, userinfo.email and gmail.send — send-only, requested now so the athlete is not asked twice. No refresh token means no connection. Reconnection reuses the durable identity; the same Google account on a different athlete is refused. Gmail transport is D5, and nothing here sends. |
| Mailbox consent capability | **The consent token travels in a URL fragment and never in a request URL** (D3.1, 2026-09-09) | D3 served the consent page at `/api/mailbox-consent/:token`, which put a live bearer capability in the request path. Measured rather than assumed: with a logging proxy in front of the real application, two of four request-log lines contained the raw token, and one contained it twice — in the request line and in `Referer`. Render documents `path` as a filterable field of its HTTP request logs, retained 7–30 days and optionally streamed onward, so the platform was never a defence. The token now sits after a `#`, which a browser never transmits: the page at `/mailbox-consent` is the same bytes for everybody, its one same-origin script reads `location.hash`, clears it with `history.replaceState`, and POSTs the token in a request body. The script is an external file, so `script-src 'self'` did not have to be loosened. Consent semantics are unchanged — 30 minutes, 256-bit secret, HMAC-only storage, single successful consumption, failed OAuth spends nothing. `state` remains the one value in a URL, because OAuth returns it in a query string, which is why it is a separate ten-minute nonce. |
| Google scope tier | **`gmail.send` is a SENSITIVE scope, not a restricted one** (D3.1, 2026-09-09) | D3 documented it as restricted and budgeted for the annual third-party security assessment. That was wrong and expensive to believe. Google's Gmail API scope table lists `gmail.send` under Sensitive; the restricted Gmail scopes are the ones that can read or alter mail. Go live needs **OAuth app verification** — required for sensitive and restricted scopes alike — and does **not** need the annual independent security assessment, which is a restricted-scope obligation. Until verification the project is capped at 100 users, a lifetime cap that cannot be reset. The scope set is unchanged; a test now names every restricted Gmail scope and asserts we ask for none of them, so drifting into that tier fails a run rather than a review. |
...
- [x] **A school spelled two ways is a school the product cannot see.**
      2026-08-28. Every join is `roster_players.college_name = colleges.name`,
      and the roster sources write a school's full official name where the
      men's registry writes a terse one — "Avila University" against "Avila",
      "Xavier University of Louisiana" against "Xavier (LA)". **79 NAIA men's
      programmes held four seasons of roster data no feature could read.**
      NAIA men's 2026 coverage 47% → 82%; pool-wide 1,837 → 1,903 of 2,145.

      Resolved in `server/lib/rosterSchoolAliases.js` as a **table, not a
      matcher**: candidates were generated by resolving identity words scoped
      hard by sport AND division, refusing anything ambiguous, then every row
      was checked against the registry entry's own city. Shipping the rule
      instead of the result would re-run heuristics on data they were never
      checked against, which is how `schoolMatch.js` corrupted three columns.
      Division is what makes it safe: Georgetown College (NAIA, Kentucky),
      University of Providence (NAIA, Montana) and St. Thomas University (NAIA,
      Florida) share every identity word with a D1 programme, and name-only
      resolution maps all three onto a flagship. Georgetown D1 still holds its
      own five seasons.

      Two traps found on the way. A trailing state word is *sometimes*
      location: "Xavier University of Louisiana" is the Xavier in Louisiana but
      "Eastern Oregon University" is not the Eastern in Oregon, so stripping it
      unconditionally lost two schools that had already resolved — it is now a
      fallback that the candidate must independently confirm by state. And
      `prior_programme` still held the pre-rename spelling, so **every returner
      read as an incoming transfer** — Georgetown (KY) showed 22 arrivals in a
      squad of 31. Re-running `project-minutes` after the rename brings it to 2,
      which is the documented median.

      The roster CSVs keep the source's own spelling; the alias translates on
      the way in. Still unjoined: **8 schools, 476 rows**, which need a
      `colleges` row rather than an alias — Graceland, Johnson University,
      Indiana University East, Brewton-Parker, Calumet, Central Christian (all
