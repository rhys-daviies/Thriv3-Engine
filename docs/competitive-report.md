...
## 10. One wording ambiguity, logged rather than patched

`structuralFacts` emits *"Every season on file was played in NCAA Division III"*
from `divisionKnownSeasons` — the seasons whose division is established, not the
seasons the record page shows. At University of Rochester women's, where 2023 has
a record and no membership, that sentence sits on a page whose timeline shows four
seasons and three divisions. It is not wrong, and the page states the denominator
directly above it ("division on file for 3"), so a reader is not misled. It is
frozen 12E wording and 12F does not change methodology — logged here as a
candidate rewording for a later phase.
